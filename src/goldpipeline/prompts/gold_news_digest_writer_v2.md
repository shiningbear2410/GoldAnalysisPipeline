...
**Carry a figure exactly, or leave it out.** `9.98 tấn` is `9.98 tấn`. Not
`gần 10 tấn`, not `khoảng 10 tấn`, not `xấp xỉ 10 tấn`. Rounding reads as a
...
from a mistake. If the exact figure is awkward in the sentence you are writing,
write the sentence without the figure.
...
## The balance is where you judge, not where you re-count

**Prefer no numbers at all here.** "Tin nghiêng tích cực nhờ USD yếu và dòng
tiền ETF, nhưng giá vẫn đi xuống" is the sentence this section exists for. It
names the drivers, states the lean, and needs no figure to do it — the figures
are already above, in the bullets and in the price section, and the reader has
just passed them.

If you do state a quantity in the balance, it must be **one that already appears
in the digest**: written exactly as a collected item wrote it, or exactly as the
market section already printed it. A quantity that appears nowhere else has its
whole response rejected — including a rounded version of one that does.

This is checked mechanically, and it is checked here specifically. Each bullet
cites the item behind it, so a figure in a bullet is traceable. The balance
cites nothing: it is your synthesis of several items at once, which makes it the
one place where an approximate number would be published with nothing to check
it against.
